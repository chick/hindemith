    ...
    def match(astNode, semanticModel, symbolTable, astVariables, astNodes, defines):
        ...
        if (classname == 'Call'):
            if (astNode.func.id == 'warp_img2d'):
                ...
                astVariables[id(astNode)] = newVar
                astNodes[id(astNode)] = astNodes[id(astNode.args[0])] + astNodes[id(astNode.args[1])] + astNodes[
                    id(astNode.args[2])] + [m]
                return True
        return False
        ...
        if (classname == 'Expr'):
            ...
            if (classname2 == 'Call'):
                if (astNode2.func.id == 'set_brox_matrix'):
                    ...
                    astNodes[id(astNode)] = astNodes[id(astNode2.args[0])] + astNodes[id(astNode2.args[1])] + astNodes[
                        id(astNode2.args[2])] + [m]
                    return True
        return False
    ...
    def match(astNode, semanticModel, symbolTable, astVariables, astNodes, defines):
        ...
        if (classname == 'Call'):
            if (astNode.func.id == 'extract_snapshots'):
                ...
                shape = (defines['N_BLOCKS'] * defines['N_DOP'], defines['TRAINING_BLOCK_SIZE'],
                         defines['N_CHAN'] * defines['TDOF'])
                newVar0 = Array3D.Create(symbolTable, uniqueName(), sym0.properties['dtype'], shape, 'Inferred', False)
    ...
    def match(astNode, semanticModel, symbolTable, astVariables, astNodes, defines):
        ...
        if (classname == 'Call'):
            if (astNode.func.id == 'cgemm_batch'):
                v0 = astVariables[id(astNode.args[0])]
                sym0 = symbolTable[v0.properties['name']]
                shape = (defines['N_BLOCKS'] * defines['N_DOP'], defines['N_CHAN'] * defines['TDOF'],
                         defines['N_CHAN'] * defines['TDOF'])
                newVar0 = Array3D.Create(symbolTable, uniqueName(), sym0.properties['dtype'], shape, 'Inferred', False)
                m = Filter('CgemmBatch')
                m.children.append(newVar0)
    ...
    def match(astNode, semanticModel, symbolTable, astVariables, astNodes, defines):
        ...
        if (classname == 'Call'):
            if (astNode.func.id == 'solve_batch'):
                ...
                sym0 = symbolTable[v0.properties['name']]
                shape = (
                defines['N_BLOCKS'] * defines['N_DOP'], defines['N_STEERING'], defines['N_CHAN'] * defines['TDOF'])
                newVar0 = Array3D.Create(symbolTable, uniqueName(), sym0.properties['dtype'], shape, 'Inferred', False)
                m = Filter('SolveBatch')
                m.children.append(newVar0)
                m.children.append(v0)

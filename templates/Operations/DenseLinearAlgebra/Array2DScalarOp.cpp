...
class Array2DScalarOpElementLevel : public ElementLevel
{
  public:
    ...
    std::string op;
    Array2DScalarOpElementLevel(Array2DOperand * dstOperand, 
                        Array2DOperand * srcOperand1,
			ScalarOperand * srcOperand2,
			std::string op) :
                        ElementLevel(), 
...
void Array2DScalarOpElementLevel::compile(std::stringstream & ss)
{
...
void Array2DScalarOpSetupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv, Stmt * stmt)
{
  ...
  // Strand
  Level * level = new Array2DScalarOpElementLevel(dstOperand, srcOperand1, srcOperand2, stmt->properties["op"]);
  EStmt * estmt = new EStmt("Array2DScalarOp", level, LaunchPartition(2, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));

...
class Array3DCopyElementLevel : public ElementLevel
{
  public:
    Array3DOperand * dst;
    Array3DOperand * src;
    Array3DCopyElementLevel(Array3DOperand * dst, 
			  Array3DOperand * src) :
			  dst(dst), 
			  src(src)
			  {}
...
void Array3DCopyElementLevel::compile(std::stringstream & ss)
{
...
void Array3DCopySetupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv, Stmt * stmt)
{
  ...
  // Strand
  Level * level = new Array3DCopyElementLevel(dstOperand, srcOperand);
  EStmt * estmt = new EStmt("Array3DCopy", level, LaunchPartition(3, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));

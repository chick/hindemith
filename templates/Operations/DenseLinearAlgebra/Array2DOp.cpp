...
class Array2DOpElementLevel : public ElementLevel
{
  public:
    ...
    std::string op;
    Array2DOpElementLevel(Array2DOperand * dstOperand, 
                        Array2DOperand * srcOperand1,
			Array2DOperand * srcOperand2,
			std::string op) :
...
void Array2DOpSetupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv, Stmt * stmt)
{
  Array2D * a1 = dynamic_cast<Array2D*>(vars[1]);
  Array2D * a2 = dynamic_cast<Array2D*>(vars[2]);
  Array2D * a3 = dynamic_cast<Array2D*>(vars[0]);
  ...
  // Operands
  Array2DOperand * dstOperand = new Array2DOperand(a3, Array2DDevicePartition( dim, ndevices, devsize, blksize, nelements, 1));
  Array2DOperand * srcOperand1 = new Array2DOperand(a1, Array2DDevicePartition( dim, ndevices, devsize, blksize, nelements, 1));
  Array2DOperand * srcOperand2 = new Array2DOperand(a2, Array2DDevicePartition( dim, ndevices, devsize, blksize, nelements, 1));

  // Create EStmt
  Level * level = new Array2DOpElementLevel(dstOperand, srcOperand1, srcOperand2, stmt->properties["op"]);
  EStmt * estmt = new EStmt("Array2DOp", level, LaunchPartition(2, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));

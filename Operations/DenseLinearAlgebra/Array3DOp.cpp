...
class Array3DOpElementLevel : public ElementLevel
{
  public:
    Array3DOperand * dstOperand;
    Array3DOperand * srcOperand1;
    Array3DOperand * srcOperand2;
    std::string op;
    Array3DOpElementLevel(Array3DOperand * dstOperand, 
                        Array3DOperand * srcOperand1,
			Array3DOperand * srcOperand2,
			std::string op) :
...
void Array3DOpElementLevel::compile(std::stringstream & ss)
{
...
void Array3DOpSetupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv, Stmt * stmt)
{
  Array3D * a1 = dynamic_cast<Array3D*>(vars[1]);
  Array3D * a2 = dynamic_cast<Array3D*>(vars[2]);
  Array3D * a3 = dynamic_cast<Array3D*>(vars[0]);
  ...
  nblocks[2] = (((dim[2] + ne2-1)/ne2 + nt2-1) / nt2);
  nthreads[0] = nt0;
  nthreads[1] = nt1;
  nthreads[2] = nt2;
  nelements[0] = ne0;
  ...
  // Operands
  Array3DOperand * dstOperand = new Array3DOperand(a3, Array3DDevicePartition( dim, ndevices, devsize, blksize, nelements, 1));
  Array3DOperand * srcOperand1 = new Array3DOperand(a1, Array3DDevicePartition( dim, ndevices, devsize, blksize, nelements, 1));
  Array3DOperand * srcOperand2 = new Array3DOperand(a2, Array3DDevicePartition( dim, ndevices, devsize, blksize, nelements, 1));

  // Create EStmt
  Level * level = new Array3DOpElementLevel(dstOperand, srcOperand1, srcOperand2, stmt->properties["op"]);
  EStmt * estmt = new EStmt("Array3DOp", level, LaunchPartition(3, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));

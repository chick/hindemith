...
class TSSyrkElementLevel : public ElementLevel
{
  public:
    Array2DOperand * dst;
    Array2DOperand * src;
    TSSyrkElementLevel(Array2DOperand * dst, 
			  Array2DOperand * src) :
			  dst(dst), 
			  src(src)
			  {}
...
void TSSyrkElementLevel::compile(std::stringstream & ss)
{
...
void TSSyrk::setupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv)
{
  ...
  // Strand
  Level * level = new TSSyrkElementLevel(dstOperand, srcOperand);
  EStmt * estmt = new EStmt("TSSyrk", level, LaunchPartition(2, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));

...
class RedArray2DElementLevel : public ElementLevel
{
  public:
    Array2DOperand * dst;
    Array2DOperand * src;
    RedArray2DElementLevel(Array2DOperand * dst, 
			  Array2DOperand * src) :
			  dst(dst), 
			  src(src)
			  {}
...
void RedArray2DElementLevel::compile(std::stringstream & ss)
{
  CodeTemplate ct = CodeTemplate();
  ct.addLine("<a> = (element_id0 + element_id1) & 0x1 ? 0.0 : <b>;");
...
void RedArray2DSetupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv, Stmt * stmt)
{
  ...
  // Strand
  Level * level = new RedArray2DElementLevel(dstOperand, srcOperand);
  EStmt * estmt = new EStmt("RedArray2D", level, LaunchPartition(2, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));

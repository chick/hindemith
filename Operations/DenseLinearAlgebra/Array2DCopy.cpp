...
class Array2DCopyElementLevel : public ElementLevel
{
  public:
    Array2DOperand * dst;
    Array2DOperand * src;
    Array2DCopyElementLevel(Array2DOperand * dst, 
			  Array2DOperand * src) :
			  dst(dst), 
			  src(src)
			  {}
...
void Array2DCopyElementLevel::compile(std::stringstream & ss)
{
...
void Array2DCopySetupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv, Stmt * stmt)
{
  ...
  // Strand
  Level * level = new Array2DCopyElementLevel(dstOperand, srcOperand);
  EStmt * estmt = new EStmt("Array2DCopy", level, LaunchPartition(2, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));

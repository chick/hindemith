...
class ArrayCopyElementLevel : public ElementLevel
{
  public:
    ArrayOperand * dst;
    ArrayOperand * src;
    ArrayCopyElementLevel(ArrayOperand * dst, 
			  ArrayOperand * src) :
			  dst(dst), 
			  src(src)
			  {}
...
void ArrayCopyElementLevel::compile(std::stringstream & ss)
{
...
void ArrayCopySetupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv, Stmt * stmt)
{
  ...
  // Strand
  Level * level = new ArrayCopyElementLevel(dstOperand, srcOperand);
  EStmt * estmt = new EStmt("ArrayCopy", level, LaunchPartition(1, dim, nblocks, nt, ne, ELEMENT));

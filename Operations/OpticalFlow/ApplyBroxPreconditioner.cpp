...
class ApplyBroxPreconditionerElementLevel : public ElementLevel
{
  public:
    Array2DOperand * dstOperand0;
    Array2DOperand * dstOperand1;
    SpMat2DOperand * stencilOperand;
    ...
    int dim0;
    int dim1;
    ApplyBroxPreconditionerElementLevel(Array2DOperand * dstOperand0,
    			Array2DOperand * dstOperand1,
    			SpMat2DOperand * stencilOperand,
			Array2DOperand * diagOperand0,
			Array2DOperand * diagOperand1,
...
void ApplyBroxPreconditionerElementLevel::compile(std::stringstream & ss)
{
  CodeTemplate ct = CodeTemplate();
  ct.addLine("float coef_a = <A> + <STENCIL_VAL>;");
  ct.addLine("float coef_d = <D> + <STENCIL_VAL>;");
  ct.addLine("float det = coef_a*coef_d-<B>*<C>;");
  ct.addLine("if(fabs(det) > 1e-4)");
  ct.addLine("{");
  ...
  ct.replace("STENCIL_VAL", stencilOperand->getDataElement("2", "0", "0"));
...
void ApplyBroxPreconditionerSetupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv, Stmt * stmt)
{
  Array2D * vec_out0 = dynamic_cast<Array2D*>(vars[0]);
  Array2D * vec_out1 = dynamic_cast<Array2D*>(vars[1]);
  SpMat2D * stencil = dynamic_cast<SpMat2D*>(vars[2]);
  ...
  SpMat2DOperand * stencilOperand = new SpMat2DOperand(stencil, SpMat2DDevicePartition( dim, ndevices, devsize, blksize, nelements, 1 ));
  ...
  // Strand
  Level * level = new ApplyBroxPreconditionerElementLevel(dstOperand0, dstOperand1, stencilOperand, diagOperand0, diagOperand1, offdiagOperand, srcOperand0, srcOperand1, dim[0], dim[1]);
  EStmt * estmt = new EStmt("ApplyBroxPreconditioner", level, LaunchPartition(2, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));

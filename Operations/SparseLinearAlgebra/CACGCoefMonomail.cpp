...
class CACGCoefMonomialElementLevel : public ElementLevel
{
  public:
    Array2DOperand * dst;
    Array2DOperand * src;
    CACGCoefMonomialElementLevel(Array2DOperand * dst, 
			  Array2DOperand * src) :
			  dst(dst), 
			  src(src)
			  {}
...
void CACGCoefMonomialElementLevel::compile(std::stringstream & ss)
{
...
void CACGCoefMonomial::setupLaunch(std::vector<EStmt*> & estmts,  std::vector<Variable*> vars, std::map<std::string, Variable*> & symbolTable, std::map<std::string, Constant*> & constantTable, cl_vars_t clv)
{
  ...
  // Strand
  Level * level = new CACGCoefMonomialElementLevel(dstOperand, srcOperand);
  EStmt * estmt = new EStmt("CACGCoefMonomial", level, LaunchPartition(2, dim, ndevices, nblocks, nthreads, nelements, ELEMENT));
